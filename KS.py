...
class Aplication(
    ...
    def login_window(self):
        self.loginWindow = Tk()
        self.loginWindow.title('Login - KonectSys Pro')
        ...
        logoImage = self.labels(
            ...
            photo=self.image(self.dataBases['config'].searchDatabase('SELECT * FROM Logo')[0][0], (500, 500))[0]
        )
        ...
        loginBtn = self.button(
            ...
            function=lambda: [
                self.openAdSense(),
                self.validating_user([userName, password, visibilityPasswordBtn], self.open_software, type_password='login', parameters={'e': ''}),
            ]
        )
    ...
    def main_window(self):
        # screen configure ===================================
        self.root = Toplevel()
        self.root.title('KonectSys Pro')
        self.root.state('zoomed')
        self.root.geometry(f'{self.root.winfo_screenwidth()}x{self.root.winfo_screenheight()}')
        self.root.configure(background="#ffffff")
        self.root.wm_protocol('WM_DELETE_WINDOW', lambda: [self.loginWindow.destroy(), self.backup_dataBaes()])
    ...
    def frame_schedule(self):
        ...
        # cheat -----------------
        labelcheat = self.labels(self.frameInputsSchedule, 'Código:', 0.34, 0.36, width=0.16, custom='optional')
        ...
        self.dateScheduleEntry.bind('<<DateEntrySelected>>', lambda e: [self.search_schedule(self.treeviewSchedule, entryPicker()[2]), self.openAdSense()])
        ...
        self.cheatScheduleEntry.bind('<KeyPress>', lambda e: self.cheatScheduleEntry.configure(
            values=[name[3] for name in self.search_barCode(informations=self.searching_list('', 2, 'ID', insert=True, index=2, information=self.cheatScheduleEntry.get()), save_seacrh=False, insert=False)]
        ))

        # delete informations -------------
        deleteInformationsInputs = self.button(
            self.frameInputsSchedule, 'apagar', 0.003, 0.87, 0.04, 0.12, function=lambda: [self.insert_informations_entrys(entryPicker()[1], self.treeviewSchedule, False), self.openAdSense()],
        ...
        # buttons management ===========================================
        functions = {
            'search': lambda: [self.search_schedule(self.treeviewSchedule, entryPicker()[0]), self.openAdSense()],
            'order': lambda e: [self.search_schedule(self.treeviewSchedule, entryPicker()[0]), self.openAdSense()],
            'update': lambda: [self.update_schedule(self.treeviewSchedule, entryPicker()[0], entryPicker()[1]), self.openAdSense()],
            'delete': lambda: [self.delete_schedule(self.treeviewSchedule), self.openAdSense()],
            'pdf': lambda: [self.create_pdf_schedule(self.treeviewSchedule), self.openAdSense()],
            'informations': lambda: [self.message_informations_schedule(self.treeviewSchedule), self.openAdSense()]
        }
        self.orderBtnSchedule = self.tab_of_buttons(0.675, 0.02, 0.3, 0.9, self.frameInputsSchedule, functions, self.photosAndIcons, informationOfTable, type_btns='management')
    ...
    def frame_scheduling(self):
        ...
        # cheat ----------
        labelCheat = self.labels(self.frameInputsScheduling, 'Código:', 0.35, 0.73, width=0.13, custom='optional')
        self.cheatSchedulingEntry = self.entry(self.frameInputsScheduling, 0.47, 0.725, 0.17, 0.12, type_entry='list')

        # events bind of frame inputs ===========================
        self.customSchedulingEntry.bind("<FocusOut>", lambda e: [
            ...
            if len([name[2] for name in self.search_service(informations=self.searching_list(self.serviceSchedulingEntry.get(), 1, 'serviço'), save_seacrh=False, insert=False)]) > 0 and self.serviceScheduleEntry.get() != '' else ''
        ])
        ...
        self.cheatSchedulingEntry.bind('<KeyPress>', lambda e: self.cheatSchedulingEntry.configure(
            values=[name[3] for name in self.search_barCode(informations=self.searching_list('', 2, 'ID', insert=True, index=2, information=self.cheatSchedulingEntry.get()), save_seacrh=False, insert=False)]
        ))

        # delete informations -------------
        deleteInformationsInputs = self.button(
            self.frameInputsScheduling, 'apagar', 0.003, 0.87, 0.04, 0.12, function=lambda: [self.insert_informations_entrys(entryPicker()[1], self.treeviewScheduling, False), self.openAdSense()],
            photo=self.image('assets/clear_inputs.png', (26, 26))[0], type_btn='buttonPhoto', background='white', hover_cursor='white'
        )
        ...
        self.cheatSchedulingEntry.bind('<Return>', lambda e: self.register_scheduling(
            entryPicker()[0], type_function='add', treeview=self.treeviewScheduling, type_insert='codeEventBind',
            entrys=[entryPicker()[1][1], entryPicker()[1][3], entryPicker()[1][5]], verification=False
        ))
                ...
                if isinstance(widget, CTkComboBox) or isinstance(widget, DateEntry) or isinstance(widget, CTkEntry):
                    match widget:
                        ...
                        case self.timeSchedulingEntry:
                            ...
                            entrys.append(widget)
                        case self.customSchedulingEntry:
                            entrysGet[0].append(widget.get().title())
                            entrys.append(widget)
                        case _:
        ...
        # add -------------
        addBtn = self.button(frameBtns.tab('Agendamento'), 'Adicionar serviço', 0.225, 0.08, 0.55, 0.2, function=lambda: [self.register_scheduling(entryPicker()[0], type_function='add', treeview=self.treeviewScheduling), self.openAdSense()])
        # remove -------------
        removeBtn = self.button(frameBtns.tab('Agendamento'), 'Remover serviço', 0.225, 0.38, 0.55, 0.2, function=lambda: [self.register_scheduling(entryPicker()[0], type_function='remove', treeview=self.treeviewScheduling), self.openAdSense()])
        # finish scheduling ---------
        registerBtn = self.button(frameBtns.tab('Agendamento'), 'Finalizar agendamento', 0.225, 0.68, 0.55, 0.2, function=lambda: [self.register_scheduling(entryPicker()[0], type_function='finishRegister', treeview=self.treeviewScheduling), self.openAdSense()])
    ...
    def frame_customers(self):
        ...
        # E-mail -------------
        labelEmail = self.labels(self.frameInputsClient, 'E-mail:', 0.02, 0.36, width=0.15)
        self.EmailClientEntry = self.entry(self.frameInputsClient, 0.1, 0.36, 0.20, 0.12, type_entry='entry')
        ...
        # selection image ---------
        labelPhoto = self.labels(self.frameInputsClient, 'Foto:', 0.32, 0.78, width=0.16, custom='optional')
        ...
        # delete informations -------------
        deleteInformationsInputs = self.button(
            self.frameInputsClient, '', 0.003, 0.87, 0.04, 0.12, function=lambda: [self.insert_informations_entrys(entryPicker()[1], self.treeviewClient, False, type_insert='advanced', table='Clientes', photo='costumer'), self.openAdSense()],
            photo=self.image('assets/clear_inputs.png', (26, 26))[0], type_btn='buttonPhoto', background='white', hover_cursor='white'
        )
        ...
        # Treeview -----------------------------------------------------
        informationOfTable = ('ID', 'Nome', 'Nascimento', 'E-mail', 'Filhos', 'Telefone', 'Cliente desde', 'Endereço', 'CEP', 'Bairro', 'Cidade', 'Estado')
        ...
        # buttons management ============
        functions = {
            'register': lambda: [self.register_client(entryPicker()[0], self.treeviewClient), self.openAdSense()],
            'search': lambda: [self.search_client(self.treeviewClient, entryPicker()[0]), self.openAdSense()],
            'order': lambda e: [self.search_client(self.treeviewClient, entryPicker()[0]), self.openAdSense()],
            'update': lambda: [self.update_client(self.treeviewClient, entryPicker()[0], entryPicker()[1]), self.openAdSense()],
            'delete': lambda: [self.delete_client(self.treeviewClient), self.openAdSense()],
            'pdf': lambda: [self.create_pdf_client(self.treeviewClient), self.openAdSense()],
            'informations': lambda: [self.message_informations_clients(self.treeviewClient), self.openAdSense()]
        }
    ...
    def frame_employers(self):
        ...
        # cpf -------------
        labelcpf = self.labels(self.frameInputsEmployee, 'Nascimento:', 0.02, 0.22, width=0.15)
        self.cpfEmployeeEntry = self.entry(self.frameInputsEmployee, 0.14, 0.22, 0.16, 0.12, type_entry='date')
        ...
        # selection image ---------
        labelPhoto = self.labels(self.frameInputsEmployee, 'Foto:', 0.32, 0.78, width=0.16, custom='optional')
        imageBtn = self.button(
        ...
        # delete informations -------------
        deleteInformationsInputs = self.button(
            self.frameInputsEmployee, 'apagar', 0.003, 0.87, 0.04, 0.12, function=lambda: [self.insert_informations_entrys(entryPicker()[1], self.treeviewClient, False, type_insert='advanced', table='Profissional', photo='employee'), self.openAdSense()],
            photo=self.image('assets/clear_inputs.png', (26, 26))[0], type_btn='buttonPhoto', background='white', hover_cursor='white'
        )
        ...
        # Treeview -----------------------------------------------------
        informationOfTable = ('ID', 'Nome', 'Nascimento', 'Admissão', 'E-mail', 'Telefone', 'Emergência', 'Endereço', 'Bairro', 'CEP', 'Cidade', 'Estado')
        ...
        # buttons management ============
        functions = {
            'register': lambda: [self.register_professional(entryPicker()[0], self.treeviewEmployer), self.openAdSense()],
            'search': lambda: [self.search_professional(self.treeviewEmployer, entryPicker()[0]), self.openAdSense()],
            'order': lambda e: [self.search_professional(self.treeviewEmployer, entryPicker()[0]), self.openAdSense()],
            'update': lambda: [self.update_professional(self.treeviewEmployer, entryPicker()[0]), self.openAdSense()],
            'delete': lambda: [self.delete_professional(self.treeviewEmployer), self.openAdSense()],
            'pdf': lambda: [self.create_pdf_professional(self.treeviewEmployer), self.openAdSense()],
            'informations': lambda: [self.message_informations_professional(self.treeviewEmployer), self.openAdSense()]
        }
    ...
    def frame_service(self):
        ...
        # delete informations -------------
        deleteInformationsInputs = self.button(
            self.frameInputsService, 'apagar', 0.003, 0.87, 0.05, 0.12, function=lambda: [self.insert_informations_entrys(entryPicker()[1], self.treeviewService, False), self.openAdSense()],
            photo=self.image('assets/clear_inputs.png', (26, 26))[0], type_btn='buttonPhoto', background='white', hover_cursor='white'
        )
        ...
        # buttons management ============
        functions = {
            'register': lambda: [self.register_service(entryPicker()[0], self.treeviewService), self.openAdSense()],
            'search': lambda: [self.search_service(self.treeviewService, entryPicker()[0]), self.openAdSense()],
            'order': lambda e: [self.search_service(self.treeviewService, entryPicker()[0]), self.openAdSense()],
            'update': lambda: [self.update_service(self.treeviewService, entryPicker()[0]), self.openAdSense()],
            'delete': lambda: [self.delete_service(self.treeviewService), self.openAdSense()],
            'pdf': lambda: [self.create_pdf_service(self.treeviewService), self.openAdSense()],
            'informations': lambda: [self.message_informations_service(self.treeviewService), self.openAdSense()]
        }
    ...
    def frame_barCode(self):
        ...
        # delete informations -------------
        deleteInformationsInputs = self.button(
            self.frameInputsBarCode, 'apagar', 0.003, 0.87, 0.04, 0.12, function=lambda: [self.insert_informations_entrys(entryPicker()[1], self.treeviewClient, False, type_insert='advanced', table='Código_de_barras', photo='barCode'), self.openAdSense()],
            photo=self.image('assets/clear_inputs.png', (26, 26))[0], type_btn='buttonPhoto', background='white', hover_cursor='white'
        )

        # make collage --------------------
        makeCollage = self.button(
            self.frameInputsBarCode, '', 0.05, 0.87, 0.03, 0.12, function=lambda: [self.create_image_colage(), self.openAdSense()],
        ...
        # buttons management ============
        functions = {
            'register': lambda: [self.register_barCode(entryPicker()[0], self.treeviewBarCode), self.openAdSense()],
            'search': lambda: [self.search_barCode(self.treeviewBarCode, entryPicker()[0]), self.openAdSense()],
            'order': lambda e: [self.search_barCode(self.treeviewBarCode, entryPicker()[0]), self.openAdSense()],
            'update': lambda: [self.update_barCode(self.treeviewBarCode, entryPicker()[0]), self.openAdSense()],
            'delete': lambda: [self.delete_barCode(self.treeviewBarCode), self.openAdSense()],
            'pdf': lambda: [self.create_pdf_barCode(self.treeviewBarCode), self.openAdSense()],
            'informations': lambda: [self.message_informations_barCode(self.treeviewBarCode), self.openAdSense()]
        }
    ...
    def frame_stock_informations(self):
        ...
        functions = {
            'register': lambda: [self.register_InformationsStock(
                ['Tipo', self.type['entry'].get(), self.type['order'].get(), 'supplier'], self.type['treeview']
            ),
                self.openAdSense()
            ],
            'search': lambda: [self.search_InformationsStock(
    ...
    def frame_use_inventory_control(self):
        ...
        # exit -----------------
        labelexit = self.labels(self.frameInputsUseInventoryControl, 'Saída:', 0.34, 0.22, width=0.16, custom='optional')
        ...
        # remainingAmount -----------------
        labelRemainingAmount = self.labels(self.frameInputsUseInventoryControl, 'Q/Restante:', 0.34, 0.36, width=0.16, custom='optional')
        ...
        # entry -----------------
        labelEntry = self.labels(self.frameInputsUseInventoryControl, 'Entrada:', 0.34, 0.50, width=0.16, custom='optional')
        self.entryUseInventoryControlEntry = self.entry(self.frameInputsUseInventoryControl, 0.44, 0.50, 0.2, 0.12, type_entry='entry')

        # modify -----------------
        labelModify = self.labels(self.frameInputsUseInventoryControl, 'Modificação:', 0.34, 0.64, width=0.16, custom='optional')
        ...
        # selection image ---------
        labelPhoto = self.labels(self.frameInputsUseInventoryControl, 'Foto:', 0.34, 0.78, width=0.16, custom='optional')
        imageBtn = self.button(
        ...
        deleteInformationsInputs = self.button(
            ...
            function=lambda: [self.insert_informations_entrys(entryPicker()[1], self.treeviewUseInventoryControl, False, type_insert='advanced', table='Estoque_de_uso', photo='productUse', data_base='stock'), self.openAdSense()],
            photo=self.image('assets/clear_inputs.png', (26, 26))[0], type_btn='buttonPhoto', background='white', hover_cursor='white'
        )
        ...
        # buttons management ============
        functions = {
            'register': lambda: [self.register_stock(entryPicker()[0], self.treeviewUseInventoryControl, sqlRegister=registerUsageStock, table='Estoque_de_uso', typeStock='productUse'), self.openAdSense()],
            'search': lambda: [self.search_stock(self.treeviewUseInventoryControl, entryPicker()[0], typeStock='productUse', sqlSearch=searchUsageStock), self.openAdSense()],
            'order': lambda e: [self.search_stock(self.treeviewUseInventoryControl, entryPicker()[0], typeStock='productUse', sqlSearch=searchUsageStock), self.openAdSense()],
            'update': lambda: [self.update_stock(self.treeviewUseInventoryControl, entryPicker()[0], typeStock='productUse', sqlUpdate=updateUsageStock), self.openAdSense()],
            'delete': lambda: [self.delete_stock(self.treeviewUseInventoryControl, table='Estoque_de_uso', typeStock='productUse', register_in_unusable=True, treeview2=self.treeviewUseInventoryControlUnusable), self.openAdSense()],
            ...
            'informations': lambda: [self.message_informations_stock(self.treeviewUseInventoryControl, typeMessage=messageUseStock, typeStock='usageStock'), self.openAdSense()]
        }
    ...
    def frame_sale_inventory_control(self):
        ...
        # custom -------------
        labelCustom = self.labels(self.frameInputsSaleInventoryControl, 'Cliente:', 0.34, 0.36, width=0.15, custom='optional')
        self.customSaleInventoryControlEntry = self.entry(self.frameInputsSaleInventoryControl, 0.44, 0.36, 0.2, 0.12, type_entry='list')

        # method pay -----------------
        labelMethodPay = self.labels(self.frameInputsSaleInventoryControl, 'M/Pagamento:', 0.34, 0.50, width=0.16, custom='optional')
        ...
        # entry -----------------
        labelEntry = self.labels(self.frameInputsSaleInventoryControl, 'Entrada:', 0.34, 0.64, width=0.16, custom='optional')
        ...
        # selection image ---------
        labelPhoto = self.labels(self.frameInputsSaleInventoryControl, 'Foto:', 0.34, 0.78, width=0.16, custom='optional')
        imageBtn = self.button(
        ...
        deleteInformationsInputs = self.button(
            ...
            function=lambda: [self.insert_informations_entrys(entryPicker()[1], self.treeviewUseInventoryControl, False, type_insert='advanced', table='Estoque_de_venda', photo='productSale', data_base='stock'), self.openAdSense()],
            photo=self.image('assets/clear_inputs.png', (26, 26))[0], type_btn='buttonPhoto', background='white', hover_cursor='white'
        )
        ...
        # buttons management ============
        functions = {
            'register': lambda: [self.register_stock(entryPicker()[0], self.treeviewSaleInventoryControl, sqlRegister=registerSaleStock, table='Estoque_de_venda', typeStock='productSale', column='entrada'), self.openAdSense()],
            'search': lambda: [self.search_stock(self.treeviewSaleInventoryControl, entryPicker()[0], typeStock='productSale', sqlSearch=searchSaleStock), self.openAdSense()],
            'order': lambda e: [self.search_stock(self.treeviewSaleInventoryControl, entryPicker()[0], typeStock='productSale', sqlSearch=searchSaleStock), self.openAdSense()],
            'update': lambda: [self.update_stock(self.treeviewSaleInventoryControl, entryPicker()[0], typeStock='productSale', sqlUpdate=updateSaleStock), self.openAdSense()],
            'sale': lambda: [
            ...
            'delete': lambda: [self.delete_stock(self.treeviewSaleInventoryControl, table='Estoque_de_venda', typeStock='productSale', column='entrada'), self.openAdSense()],
            'pdf': lambda: [self.create_pdf_stock(
            ...
            'informations': lambda: [self.message_informations_stock(self.treeviewSaleInventoryControl, typeMessage=messageSaleStock, typeStock='saleStock'), self.openAdSense()]
        }
    ...
    def frame_use_inventory_control_unusable(self):
        ...
        # exit -----------------
        labelExit = self.labels(self.frameInputsUseInventoryControlUnusable, 'Saída:', 0.34, 0.22, width=0.16, custom='optional')
        ...
        # remainingAmount -----------------
        labelRemainingAmount = self.labels(self.frameInputsUseInventoryControlUnusable, 'Q/Restante:', 0.34, 0.36, width=0.16, custom='optional')
        ...
        # date exit -----------------
        labelDateExit = self.labels(self.frameInputsUseInventoryControlUnusable, 'D/Saída:', 0.34, 0.50, width=0.16, custom='optional')
        ...
        # modify -----------------
        labelModify = self.labels(self.frameInputsUseInventoryControlUnusable, 'Modificação:', 0.34, 0.64, width=0.16, custom='optional')
        ...
        # selection image ---------
        labelPhoto = self.labels(self.frameInputsUseInventoryControlUnusable, 'Foto:', 0.34, 0.78, width=0.16, custom='optional')
        imageBtn = self.button(
    ...
    def frame_sale_inventory_control_unusable(self):
        ...
        # custom -------------
        labelCustom = self.labels(self.frameInputsSaleInventoryControlUnusable, 'Cliente:', 0.34, 0.36, width=0.15, custom='optional')
        ...
        # sold -----------------
        labelMethodPay = self.labels(self.frameInputsSaleInventoryControlUnusable, 'M/Pagamento:', 0.34, 0.50, width=0.16, custom='optional')
        ...
        # modify -----------------
        labelold = self.labels(self.frameInputsSaleInventoryControlUnusable, 'Venda:', 0.34, 0.64, width=0.16, custom='optional')
        ...
        # selection image ---------
        labelPhoto = self.labels(self.frameInputsSaleInventoryControlUnusable, 'Foto:', 0.34, 0.78, width=0.16, custom='optional')
        imageBtn = self.button(
    ...
    def frame_cash_register_management_day(self):
        ...
        # status -----------------
        labelStatus = self.labels(self.frameInputsCashDay, 'Status:', 0.34, 0.50, width=0.16, custom='optional')
        ...
        # exit -----------------
        labelExit = self.labels(self.frameInputsCashDay, 'Saida:', 0.34, 0.64, width=0.16, custom='optional')
        self.exitDayEntry = self.entry(self.frameInputsCashDay, 0.44, 0.64, 0.2, 0.12, type_entry='entry')

        # metody exit -----------------
        labelMetodyExit = self.labels(self.frameInputsCashDay, 'M/Saida:', 0.34, 0.78, width=0.16, custom='optional')
    ...
    def frame_cash_register_management_month(self):
        ...
        # status -----------------
        labelStatus = self.labels(self.frameInputsCashMonth, 'Status:', 0.34, 0.50, width=0.16, custom='optional')
        ...
        # exit -----------------
        labelExit = self.labels(self.frameInputsCashMonth, 'Saida:', 0.34, 0.64, width=0.16, custom='optional')
        self.exitMonthEntry = self.entry(self.frameInputsCashMonth, 0.44, 0.64, 0.2, 0.12, type_entry='entry')

        # metody exit -----------------
        labelMetodyExit = self.labels(self.frameInputsCashMonth, 'M/Saida:', 0.34, 0.78, width=0.16, custom='optional')
    ...
    def frame_pay_management(self):
        ...
        self.treeviewCashPayment.configure()
        # event bind treeview ==========================================
