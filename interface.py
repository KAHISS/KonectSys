...
class Interface(GeneralFunctions):
    ...
    def main_frame_notebook(self, locate, name_tab, bg='white'):
        frame = Frame(locate, background=bg)
        frame.place(relx=0, rely=0, relwidth=1, relheight=1)
        locate.add(frame, text=name_tab, padding=[5, 0])
        return frame

    def tabview(self, locate, posx, posy, width, height, background='#b59b50', border='#b59b50'):
        tab = CTkTabview(
    ...
    def tab_of_buttons(
        ...
        if treeview == 'yes':
            ...
            # order --------------
            labelOrder = self.labels(frameBtns.tab('Tabela'), 'Ordem', 0.25, 0.0, width=0.50, height=0.2, position=CENTER, custom='tabview')
            ...
            labelInfo = self.labels(frameBtns.tab('Tabela'), 'Informações', 0.17, 0.45, width=0.7, height=0.2, color='#b59b50', background='#e8d499', position=CENTER, custom='tabview')
